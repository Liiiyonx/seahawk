...
| 地图 | 天地图 API | 政务合规友好 |
| 检测模型 | YOLO11 + ONNX Runtime / TensorRT | 边缘端部署 |
...
make test-edge         # 时序校验 11 项：单帧不确认、抽帧压噪、抑制率界内、
                       # 以及「代码默认值 vs 设计文档参数表」一致性守卫
...
| 命令 | 守住什么 | 典型能抓到的错 |
| --- | --- | --- |
| `make test-edge` | **算法逻辑 + 参数真源** | 抑制率越界（曾出现 140.9%）、抽帧误杀真实目标、`max_misses` 文档写 5 代码写 6 |
